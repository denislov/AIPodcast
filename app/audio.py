...
class AudioGenerator:
    def __init__(self):
        self.json_locks = {}  # 线程锁字典，用于防止同时写入同一个JSON文件

    # 调用API来生成音频
    def generate_audio(self, text: str, max_retries=3):
    ...
    # 更新JSON文件中的数据
    def update_json_with_audio_path(self, chapter_file_path, item_id, audio_path):
        # 获取或创建该文件的锁
        if chapter_file_path not in self.json_locks:
            self.json_locks[chapter_file_path] = threading.Lock()

        # 使用锁确保线程安全
        with self.json_locks[chapter_file_path]:
            try:
                # 读取JSON文件
    ...
    # 处理单个条目
    def process_item(self, item, book_id, chapter_file_path, pbar):
        item_id = item["id"]
        text = item["text"]
        ...
        if os.path.exists(audio_path):
            ...
            if "audio_path" not in item:
                ...
                self.update_json_with_audio_path(chapter_file_path, item_id, relative_audio_path)
            pbar.update(1)  # 更新进度条
            return True
        ...
        if audio_data is None:
            print(f"处理项目 {chapter_name}/{item_id} 失败，跳过")
            pbar.update(1)  # 更新进度条
            return False
    ...
    def create_book_audio(self, book_id: str):

        if os.path.exists(f"data/book/{book_id}/audio/.completed"):
            print(f"音频已生成，跳过")
            return True
        # 从环境变量获取线程数
        try:
                    ...
                    try:
                        ...
                        for item in chapter_data:
                            future = executor.submit(
                                self.process_item, item, book_id, chapter_file_path, pbar
                            )
                        ...
                        print(f"处理章节 {chapter_file_path} 失败：{str(e)}")


if __name__ == "__main__":
    audio_generator = AudioGenerator()
    audio_generator.create_book_audio("1043294775")
