...
def generate_board(book_id: str):
    # 确保目标目录存在
    if os.path.exists(f"data/book/{book_id}/storyboard/.completed"):
        print(f"目标目录 {book_id}/storyboard 任务已完成，跳过处理")
        return True
    ...
    # 获取所有章节文件
    chapter_files = [f for f in os.listdir(f"data/book/{book_id}/list") if f.endswith(".txt")]
    # 按文件名排序
